...
async function testCompression(endpoint) {
  ...
  try {
    ...
    if (contentEncoding && ['gzip', 'deflate'].includes(contentEncoding)) {
      ...
      console.log('✅ Response is smaller, but no Content-Encoding header (compression may be happening at another layer)');
    } else {
...
async function runTests() {
  try {
    ...
    const overallRatio = totalUncompressed > 0 ? 
                        (1 - (totalCompressed / totalUncompressed)) * 100 : 0;
