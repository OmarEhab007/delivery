...
async function testUserQueries() {
  ...
  try {
    ...
    const emailIndexUsed = emailQuery.executionStats.executionStages.inputStage?.indexName === 'email_1';
    ...
    const roleIndexUsed = roleQuery.executionStats.executionStages.inputStage?.indexName === 'role_1';
    ...
    const compoundIndexUsed = compoundQuery.executionStats.executionStages.inputStage?.indexName === 'role_1_verificationStatus_1';
...
async function testShipmentQueries() {
  ...
  try {
    ...
    const statusQuery = await Shipment.find({ status: 'in_transit' }).limit(5).explain('executionStats');
    
    const statusIndexUsed = statusQuery.executionStats.executionStages.inputStage?.indexName === 'status_1';
    ...
    const sortQuery = await Shipment.find().sort({ createdAt: -1 }).limit(5).explain('executionStats');
    
    const sortIndexUsed = sortQuery.executionStats.executionStages.inputStage?.indexName === 'createdAt_-1';
    ...
    const compoundQuery = await Shipment.find({ 
      status: 'pending' 
    }).sort({ createdAt: -1 }).limit(5).explain('executionStats');
    
    const compoundIndexUsed = compoundQuery.executionStats.executionStages.inputStage?.indexName === 'status_1_createdAt_-1';
    console.log(`Compound query used index: ${compoundIndexUsed ? 'Yes' : 'No'}`);
    console.log(`Documents examined: ${compoundQuery.executionStats.totalDocsExamined}`);
    console.log(`Execution time: ${compoundQuery.executionStats.executionTimeMillis}ms`);
...
async function runTests() {
  ...
  try {
    ...
    const userSuccess = userResults.emailIndexUsed && userResults.roleIndexUsed && userResults.compoundIndexUsed;
    const shipmentSuccess = shipmentResults.statusIndexUsed && shipmentResults.sortIndexUsed && shipmentResults.compoundIndexUsed;
