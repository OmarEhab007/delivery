...
import { useTruckOwnerAvailableShipments } from '@/hooks/use-truck-owner';

export default function TruckOwnerShipmentsPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');

  const { data, isLoading } = useTruckOwnerAvailableShipments({
    page,
    limit: 10,
  ...
  const pagination = {
    total: data?.data.total || shipments.length,
