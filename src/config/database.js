...
const connectDB = async (retryCount = 0) => {
  try {
    ...
    // Connect to database
    logger.info(
      `Connecting to MongoDB: ${process.env.MONGODB_URI?.split('@').pop() || '[URI not set]'}`
    );
    ...
    if (retryCount < MAX_RETRIES) {
      logger.info(
        `Retrying connection in ${RETRY_INTERVAL / 1000} seconds... (Attempt ${retryCount + 1}/${MAX_RETRIES})`
      );
