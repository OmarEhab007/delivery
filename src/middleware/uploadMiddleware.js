...
const singleUpload = (fieldName) => (req, res, next) => {
  ...
  logger.debug('Initial request body', { bodyKeys: Object.keys(req.body || {}) });

  const uploadSingle = upload.single(fieldName);

  uploadSingle(req, res, (err) => {
    logger.debug('After upload - request body:', req.body);
    ...
    if (req.body.entityType) {
      const relativePath = moveFileToCorrectLocation(
        req.file,
        req.body.entityType,
        req.body.entityId
      );
    ...
    logger.info(
      `Updated file details: ${JSON.stringify({
        path: req.file.path,
