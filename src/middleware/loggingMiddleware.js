...
const requestLogger = (req, res, next) => {
  ...
  res.send = function(body) {
    ...
    if (res.statusCode >= 500) {
      logger.error(`API Response: ${req.method} ${req.originalUrl || req.url} ${res.statusCode}`, responseLog);
    } else if (res.statusCode >= 400) {
      logger.warn(`API Response: ${req.method} ${req.originalUrl || req.url} ${res.statusCode}`, responseLog);
    } else {
      logger.info(`API Response: ${req.method} ${req.originalUrl || req.url} ${res.statusCode}`, responseLog);
    }
